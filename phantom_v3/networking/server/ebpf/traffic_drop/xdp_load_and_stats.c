...
static const char *default_progsec = "xdp_main1";
...
static const struct option_wrapper long_options[] = {
  ...
  {{"dropnum",    required_argument,	NULL,  'D'  },
	 "Drop except every dropnum # of packet"},
...
int main(int argc, char **argv)
{
  ...
  // set drop frequency from user space
	drop_map_fd = find_map_fd(bpf_obj, "xdp_drop_map");
	if (stats_map_fd < 0) {
		xdp_link_detach(cfg.ifindex, cfg.xdp_flags, 0);
		return EXIT_FAIL_BPF;
	}
  
  __u32 key = KEY_DROP_FREQ;
  long value = cfg.dropnum; // long type, int wont work
  printf("Accept %ldth SYN packet for every %ld packets\n", value, value);
  err = bpf_map_update_elem(drop_map_fd, &key, &value, BPF_ANY);
  if (err) {
    fprintf(stderr, "ERR: update cfg map failed\n");
  }
